...
void fn_1_155C(void) {
    ...
    if (var_r30 > 0) {
        ...
        BOARD_WIN_INSERT_MES_SET_PTR(MAKE_MESSID_PTR(sp8), 1);
    } else {
        BoardWinCreate(2, MAKE_MESSID(39, 3), 7);
        BOARD_WIN_INSERT_MES_SET_PTR(MAKE_MESSID_PTR(sp18), 0);
    }
    BoardWinWait();
    BoardWinKill();
    if (var_r30 > 0) {
...
void fn_1_1E44(void) {
    ...
    if (lbl_1_bss_0->unk_00 == 0) {
        ...
        BOARD_WIN_INSERT_MES_SET_PTR(MAKE_MESSID_PTR(sp84), 0);
        BOARD_WIN_INSERT_MES_SET_PTR(MAKE_MESSID_PTR(sp74), 1);
        BoardWinWait();
        BoardWinKill();
