...
DataListSprite lbl_1_data_118[] = {
	{ 0x4B0022, 0, 10, 288, -200, { 255, 255, 255, 255 } },
	{ 0, 0, 0, 0, 0, { 0, 0, 0, 0 } }
};

float lbl_1_data_140[] = {
	90, 250, 420
};
...
void fn_1_470(void)
{
	...
	HuWinMesMaxSizeGet(1, sp8, 0x1C0001);
	temp_r30 = HuWinExCreateStyled(-10000, 340, sp8[0], sp8[1], -1, 1);
	HuWinExAnimIn(temp_r30);
	HuWinMesPalSet(temp_r30, 7, 0, 0, 0);
	fn_1_6D80(temp_r30);
	HuWinMesSet(temp_r30, 0x1C0001);
	HuWinMesWait(temp_r30);
	HuWinExAnimOut(temp_r30);
	HuWinExCleanup(temp_r30);
	HuWinComKeyReset();
	...
	HuWinMesMaxSizeGet(1, sp8, 0x1C0003);
	temp_r30 = HuWinExCreateStyled(-10000, 340, sp8[0], sp8[1], -1, 1);
	HuWinExAnimIn(temp_r30);
	HuWinMesPalSet(temp_r30, 7, 0, 0, 0);
	fn_1_6D80(temp_r30);
	...
	switch(lbl_1_bss_2D2) {
		...
		case 1:
			if(GWPlayer[lbl_1_bss_2D6].coins <= 0 && GWPlayer[lbl_1_bss_2D4].coins <= 0
				&& GWPlayer[lbl_1_bss_2D6].stars <= 0 && GWPlayer[lbl_1_bss_2D4].stars <= 0) {
				temp_r25 = 0;
				temp_r28 = 0x1C0017;
			} else {
				temp_r28 = 0x1C0006;
...
void fn_1_3B7C(void)
{
	...
	HuWinMesMaxSizeGet(1, sp8, 0x1C0005);
...
void fn_1_655C(void)
{
	...
	Hu3DReflectMapSet(HuDataSelHeapReadNum(0x4B0023, MEMORY_DEFAULT_NUM, HEAP_DATA));
	for(i=0; i<4; i++) {
	...
	lbl_1_bss_2D0 = model;
	lbl_1_bss_2C0[0] = CharModelMotionCreate(lbl_1_bss_2DA, 0x5F0000);
	lbl_1_bss_2C0[1] = CharModelMotionCreate(lbl_1_bss_2DA, 0x5F0002);
	lbl_1_bss_2C0[2] = CharModelMotionCreate(lbl_1_bss_2DA, lbl_1_bss_2DA+0x4B0045);
	lbl_1_bss_2C0[3] = CharModelMotionCreate(lbl_1_bss_2DA, lbl_1_bss_2DA+0x4B004D);
	...
	for(i=0; i<20; i++) {
		if(i == 0) {
			model = Hu3DModelCreateFile(0x4B0013);
		} else {
	...
	for(i=0; i<20; i++) {
		if(i == 0) {
			model = Hu3DModelCreateFile(0x4B0012);
		} else {
