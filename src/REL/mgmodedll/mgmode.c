...
CameraViewParams lbl_1_data_0[1] = {
	{
		{ -5, 0, 0 },
		{ 0, 125, 0 },
		1750
...
void fn_1_464(void)
{
	...
	if(lbl_1_bss_4 == 0) {
		...
		if(lbl_1_bss_4 == 1 || lbl_1_bss_4 == 2 || lbl_1_bss_4 == 3 || lbl_1_bss_4 == 4) {
			...
			Hu3DModelAttrSet(lbl_1_bss_2C80[5], 0x40000001);
			HuPrcVSleep();
		}
	}
	fn_1_6F8();
...
void fn_1_6F8(void)
{
	...
	Hu3DModelAttrSet(lbl_1_bss_2C80[9], 0x40000002);
	...
	for(time=0; time<=30; time++) {
		for(i=0; i<4; i++) {
			if(time > i*2 && time < (i*2)+20) {
				...
				Hu3DModelAttrReset(lbl_1_bss_2C80[i+9], 1);
			}
		}
		HuPrcVSleep();
	}
...
void fn_1_DF0(void)
{
	...
	while(1) {
		...
		while(1) {
			...
			if(HuPadBtnDown[0] & PAD_BUTTON_A) {
				HuAudFXPlay(2);
				if(lbl_1_bss_2A4 == 0 && lbl_1_bss_20AC[0] == 0 && lbl_1_bss_20AC[1] == 0 && lbl_1_bss_20AC[2] == 0 && lbl_1_bss_20AC[3] == 0 && lbl_1_bss_20AC[4] == 0) {
...
s32 fn_1_1B0C(void)
{
	...
	if(lbl_1_bss_4 == 1) {
		...
		Hu3DModelAttrSet(lbl_1_bss_2C80[9], 0x40000002);
		Hu3DModelAttrReset(lbl_1_bss_2C80[9], 1);
	} else {
		Hu3DModelAttrSet(lbl_1_bss_2C80[9], 0x40000004);
...
s32 fn_1_487C(void)
{
	...
	for(i=0; i<4; i++) {
		strcpy(&str[i][0], "\x1E\x03""0""\x1E\x08");
	}
...
void fn_1_52DC(void)
{
	...
	Hu3DModelAttrSet(lbl_1_bss_2C80[0], 0x40000002);
