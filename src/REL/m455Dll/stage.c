...
void fn_1_7368(omObjData *object)
{
    s32 i;
    for(i=0; i<object->mdlcnt; i++) {
        HU3DMODEL *modelP = &Hu3DData[object->model[i]];
...
s32 fn_1_7434(Vec *pos, float scale)
{
    ...
    for(i=0; i<lbl_1_bss_64->mdlcnt; i++) {
        HU3DMODEL *modelP = &Hu3DData[lbl_1_bss_64->model[i]];
        if(modelP->attr & HU3D_ATTR_DISPOFF) {
            break;
        }
    }
...
void fn_1_76B0(omObjData *object)
{
    HU3DMODEL *modelP = &Hu3DData[object->model[0]];
...
{
    s32 i;
    HU3DMODEL *modelP = &Hu3DData[lbl_1_bss_60->model[0]];
